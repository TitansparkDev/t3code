...
4. Ensure `main` is green in CI and bump the app version as needed.
5. For the fork desktop release, create and push a tag such as `vX.Y.Z`.
6. Build the Linux and Windows installers into one directory, then publish and verify them with:
...
7. Verify workflow steps when using the hosted release workflow:
   - preflight passes
