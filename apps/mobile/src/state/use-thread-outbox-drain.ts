...
async function sendQueuedMessage(
  ...
  const serverConfig = state.registry
  ...
  const currentConfig = state.registry
    .get(environmentServerConfigsAtom)
    .get(queuedMessage.environmentId);
  if (!currentConfig) {
    return false;
...
async function sendQueuedCreation(
  ...
  // Record before queue cleanup so the thread screen never sees a gap between
  // delivery and the server's shell projection.
...
function drainOnce(state: ThreadOutboxDrainState): void {
  ...
  const creationOutcomes = registry.get(pendingThreadCreationOutcomesAtom);

  // A delivered pending task keeps a small setup bridge visible until its
  // shell has a turn (or reaches a terminal session state). Perform this
  // cleanup from the shared dispatcher so UI and headless owners agree.
  for (const [threadKey, outcome] of Object.entries(creationOutcomes)) {
    if (
      outcome.kind === "delivered" &&
      threads.some(
        (candidate) =>
          scopedThreadKey(candidate.environmentId, candidate.id) === threadKey &&
