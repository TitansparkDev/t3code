  ...
  for (const [index, payload] of input.payloads.entries()) {
    ...
    try {
      if (input.fileReader.persistFile) {
        // Shared files are temporary. Copy them into durable app-owned storage
        // so the composer remains valid after the native handoff is cleared.
      ...
      // Readers without persistence retain the legacy inline shape.
